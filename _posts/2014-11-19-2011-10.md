---
title: "2011-10"
date: 2014-11-19 12:04:25 +0000
categories: ["Nginx [2]"]
tags: ["Pages"]
description: "Wednesday, November 19, 2014 12:04 PM 30 （总结）Nginx使用的php-fpm的两种进程管理方式及优化 发表于 : Linux , Nginx , UNIX , 个人日记 , 原创总结 | 作者 : 谋万世全局者 标签 : Linux , Nginx , php-fpm , 优"
...
由上面一段文字可知，对于进程的管理存在两种风格
——static
和
dynamic
。和之前的版本的进程管理其实还是一样的，只是将
apache-like
改成了
dynamic
，这样更容易理解。
...
（静态）或者
dynamic
（动态）。在更老一些的版本中，dynamic被称作apache-like。这个要注意看配置文件的说明。
下面4个参数的意思分别为：
pm.max_children
：静态方式下开启的
php-fpm
