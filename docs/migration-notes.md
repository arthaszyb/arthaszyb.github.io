...
## 内容质量优化（batch content optimization）

对全部 398 篇文章执行了逐篇内容优化（规范见 `docs/content-editing-guide.md`，逐篇日志见 `docs/content-edit-log/batch*.jsonl`）：

- **类型分布**：转载 148、普通 140、贫瘠 100、重复/其他若干；2 篇 2025 手写文按规范跳过。
- **转载压缩**：122 篇标注 condensed，压缩为要点笔记体（硬信息全保留），135 篇写入 origin_url 溯源，文章页渲染「本文为摘录整理笔记」出处提示框。
- **贫瘠补写**：脚本/命令类补 2-4 句事实性说明。
- **标签重建**：从受控词表收敛为每篇 2-5 个，剔除历史误标（如时事文误挂 网络排查 改为 时评/历史）。
- **草稿**：25 篇正文实质为空/重复/无价值内容设 draft:true（不进列表/RSS/sitemap）。
- **终局校验**：全站 en-note/抓取垃圾零残留（正当代码块内 HTML 除外）、全 UTF-8、astro build 零错误。

执行方式：Haiku 分 27 批逐篇编辑，Fable 逐波验收后提交。网络/端口扫描/代理工具等主题偶发内容过滤误拦，由 Fable 手工接手（nmap、TCP 并发、ss、wireshark、shadowsocks、curl 批量下载、piranha 等篇）。

已知遗留（不阻塞）：部分老文仍有迁移期逐行断行伪影；repost-no-url（15 篇）原文链接已不可考，按规范保守保留内容。
