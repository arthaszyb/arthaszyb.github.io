...
| 新文章 `2026-07-23-how-ai-4-comany-init.md` 没写 `description` | meta description / og / JSON-LD / RSS 全部回退到站点默认的**英文**句子，中文查询匹配不上，搜索结果摘要也没有可读性 | 补上中文 description |
| 同一篇文章有**两个 `<h1>`** | 版式渲染 `<h1>{title}</h1>`，正文开头又写了一个 `# 任务拆解与协作篇…`，主题信号自相矛盾 | 删掉正文里重复的 H1，把副标题并进 frontmatter 的 `title` |
...
- **这篇文章的正文里，「AI 赋能」只出现在标题和「系列第 3 篇」那一行**，正文通篇讲的是
  任务拆解、spec-first、波次执行。用「如何给公司做 AI 赋能」去搜，即使被收录了，
  相关性也偏弱。建议在开头加一段真正回答"给公司做 AI 赋能是怎么回事"的引言。
- **文章自称《给公司做 AI 赋能》系列第 3 篇，但站上没有第 1、2 篇。** 对读者和爬虫都是
  断链。要么补上前两篇并互相内链（系列内链对收录很有帮助），要么去掉"第 3 篇"的说法。
- **frontmatter 里 `date: 2026-07-15`，但文件名和实际提交都是 2026-07-23。**
  `article:published_time` 和 sitemap 的 `lastmod` 都会用 07-15，比实际发布早 8 天。
  如果 07-23 才是真实发布日期，改过来能让这篇在 sitemap 里排到更前面。未擅自改动。
