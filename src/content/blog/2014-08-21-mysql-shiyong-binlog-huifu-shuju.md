...
date: '2014-08-21'
description: "两种通过binlog恢复MySQL数据的实现方式，包括Bash脚本循环处理binlog文件和Perl脚本按时间范围提取指定表的SQL操作。"
category: database
tags:
  - mysql
draft: false
...
lang: zh
---
通过MySQL的binlog进行数据恢复需要遍历binlog文件并提取指定时间段的SQL操作。以下是两种实现方式。

## 方法一：Bash脚本

逐个处理binlog文件，提取指定的操作记录：
...
do
  ...
  if [[ -f $filename ]];then
    echo "$filename"
    # 注：不指定临时目录时，大量临时数据会产生在/tmp，容易引起磁盘爆满
    # 使用--local-load指定临时目录：
