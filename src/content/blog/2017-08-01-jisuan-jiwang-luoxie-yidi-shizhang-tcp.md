...
Server和window 2003的client三步握手和注册信令都完成，但是后续的信令却迟迟无法得到回复，因此server端超时过后会主动断开连接，这就是解释了为什么反复断开连接的原因。
...
linux中通过/proc/sys/net/ipv4/tcp_window_scaling参数可以取消window scaling的机制，经过测试发送通信正常，也再次印证了上面的结论。
