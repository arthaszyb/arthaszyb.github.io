...
date: '2017-03-06'
description: >-
  星期一, 三月 6, 2017 11:33 上午 已剪辑自: http://coolshell.cn/articles/11564.html
...
已剪辑自: [http://coolshell.cn/articles/11564.html](http://coolshell.cn/articles/11564.html)

![](/images/legacy/legacy-55ecde4b37.jpg)

TCP是一个巨复杂的协议，因为他要解决很多问题，而这些问题又带出了很多子问题和阴暗面。所以学习TCP本身是个比较痛苦的过程，但对于学习的过程却能让人有很多收获。关于TCP这个协议的细节，我还是推荐你去看[W.Richard Stevens](http://www.kohala.com/start/)的《[TCP/IP](http://book.douban.com/subject/1088054/) [](http://book.douban.com/subject/1088054/)[详解](http://book.douban.com/subject/1088054/) [](http://book.douban.com/subject/1088054/)[卷](http://book.douban.com/subject/1088054/)[1](http://book.douban.com/subject/1088054/)[：协议](http://book.douban.com/subject/1088054/)》（当然，你也可以去读一下[RFC793](http://tools.ietf.org/html/rfc793)以及后面N多的RFC）。另外，本文我会使用英文术语，这样方便你通过这些英文关键词来查找相关的技术文档。
...
![](/images/legacy/legacy-83de9569b7.jpg)
...
![](/images/legacy/legacy-1439b06430.jpg)
...
![](/images/legacy/legacy-132d805fd1.png)

![](/images/legacy/legacy-87107ddc33.jpg)
...
![](/images/legacy/legacy-1301d829de.png)
...
- **关于ISN的初始化**。ISN是不能hard code的，不然会出问题的——比如：如果连接建好后始终用1来做ISN，如果client发了30个segment过去，但是网络断了，于是 client重连，又用了1做ISN，但是之前连接的那些包到了，于是就被当成了新连接的包，此时，client的Sequence Number 可能是3，而Server端认为client端的这个号是30了。全乱了。[RFC793](http://tools.ietf.org/html/rfc793)中说，ISN会和一个假的时钟绑在一起，这个时钟会在每4微秒对ISN做加一操作，直到超过2^32，又从0开始。这样，一个ISN的周期大约是4.55个小时。因为，我们假设我们的TCP Segment在网络上的存活时间不会超过Maximum Segment Lifetime（缩写为MSL – [Wikipedia](http://en.wikipedia.org/wiki/Maximum_Segment_Lifetime)[语条](http://en.wikipedia.org/wiki/Maximum_Segment_Lifetime)），所以，只要MSL的值小于4.55小时，那么，我们就不会重用到ISN。
...
其实，TIME\_WAIT表示的是你主动断连接，所以，这就是所谓的“不作死不会死”。试想，如果让对端断连接，那么这个破问题就是对方的了，呵呵。另外，如果你的服务器是于HTTP服务器，那么设置一个[HTTP](http://en.wikipedia.org/wiki/HTTP_persistent_connection)[的](http://en.wikipedia.org/wiki/HTTP_persistent_connection)[KeepAlive](http://en.wikipedia.org/wiki/HTTP_persistent_connection)有多重要（浏览器会重用一个TCP连接来处理多个HTTP请求），然后让客户端去断链接（你要小心，浏览器可能会非常贪婪，他们不到万不得已不会主动断连接）。

#### _数据传输中的Sequence Number_

下图是我从Wireshark中截了个我在访问coolshell.cn时的有数据传输的图给你看一下，SeqNum是怎么变的。（使用Wireshark菜单中的Statistics ->Flow Graph… ）

![](/images/legacy/legacy-f5b82dc94b.jpg)
...
![](/images/legacy/legacy-3da9be0bcc.png)
...
![](/images/legacy/legacy-94846e0c4e.jpg)
...
注意：SACK会消费发送方的资源，试想，如果一个攻击者给数据发送方发一堆SACK的选项，这会导致发送方开始要重传甚至遍历已经发出的数据，这会消耗很多发送端的资源。详细的东西请参看《[TCP SACK](http://www.ibm.com/developerworks/cn/linux/l-tcp-sack/)[的性能权衡](http://www.ibm.com/developerworks/cn/linux/l-tcp-sack/)》
...
<table><tbody><tr><td><div><span><span>1</span></span></div><div><span><span>2</span></span></div><div><span><span>3</span></span></div><div><span><span>4</span></span></div><div><span><span>5</span></span></div><div><span><span>6</span></span></div><div><span><span>7</span></span></div></td><td><div><span><span>Transmitted Received ACK Sent</span></span></div><div><span><span>Segment Segment (Including SACK Blocks)</span></span></div><div><span><span>3000-3499 3000-3499 3500 (ACK dropped)</span></span></div><div><span><span>3500-3999 3500-3999 4000 (ACK dropped)</span></span></div><div><span><span>3000-3499 3000-3499 4000, SACK=3000-3500</span></span></div><div><span><span> ---------</span></span></div></td></tr></tbody></table>
...
<table><tbody><tr><td><div><span><span>1</span></span></div><div><span><span>2</span></span></div><div><span><span>3</span></span></div><div><span><span>4</span></span></div><div><span><span>5</span></span></div><div><span><span>6</span></span></div><div><span><span>7</span></span></div><div><span><span>8</span></span></div><div><span><span>9</span></span></div><div><span><span>10</span></span></div><div><span><span>11</span></span></div></td><td><div><span><span>Transmitted Received ACK Sent</span></span></div><div><span><span>Segment Segment (Including SACK Blocks)</span></span></div><div><span><span>500-999 500-999 1000</span></span></div><div><span><span>1000-1499 (delayed)</span></span></div><div><span><span>1500-1999 1500-1999 1000, SACK=1500-2000</span></span></div><div><span><span>2000-2499 2000-2499 1000, SACK=1500-2500</span></span></div><div><span><span>2500-2999 2500-2999 1000, SACK=1500-3000</span></span></div><div><span><span>1000-1499 1000-1499 3000</span></span></div><div><span><span> 1000-1499 3000, SACK=1000-1500</span></span></div><div><span><span> ---------</span></span></div></td></tr></tbody></table>
...
好了，上篇就到这里结束了。如果你觉得我写得还比较浅显易懂，那么，欢迎移步看下篇《[TCP](http://coolshell.cn/articles/11609.html)[的那些事（下）](http://coolshell.cn/articles/11609.html)》

**[TCP](http://coolshell.cn/articles/11609.html)[的那些事儿（下）](http://coolshell.cn/articles/11609.html)[\>>>](http://coolshell.cn/articles/11609.html)**

（上篇完）

![](/images/legacy/legacy-54d88769f0.png)

[http://cn.udacity.com/android/?utm\_source=coolshell&utm\_medium=referral&utm\_campaign=newAND](http://cn.udacity.com/android/?utm_source=coolshell&utm_medium=referral&utm_campaign=newAND)

![](/images/legacy/legacy-fe7e2d676f.jpg)
...
![](/images/legacy/legacy-414d24f9fe.gif)
