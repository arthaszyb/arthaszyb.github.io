...
title: RHCS5维护
date: '2013-10-14'
description: RHCS 集群的启动、关闭、应用服务管理、状态监控，以及 GFS2 文件系统的维护操作。涵盖核心工具 cman、rgmanager、GFS2 管理命令的使用方法。
category: web-infra
tags:
  - 集群
  - 高可用
...
可以通过 /var/log/messages 文件查看启动应用服务的详细信息。当webserver启动后，与服务相关的集群资源（如虚拟IP、应用程序服务脚本）也随之启动。
2. 关闭某个应用服务
...
可以通过 /var/log/messages 文件查看关闭应用服务的详细信息。当mysqlserver关闭后，与服务相关的集群资源（如虚拟IP、应用程序服务脚本）也随之释放。
3. 重启某个应用服务
...
注意：此命令在web2节点上执行，但可成功重启web1节点的webserver。这说明 clusvcadm 命令在集群任意节点执行都有效。
4. 切换某个服务
...
gfs2_jadd 主要用来配置 GFS2 的 journals 数量和大小。用法：
