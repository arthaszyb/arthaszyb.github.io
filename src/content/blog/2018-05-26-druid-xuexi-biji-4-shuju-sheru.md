...
- Tranquility Kafka：发送方可以先将数据发送到Kafka,Tranquility Kafka会根据配置从Kafka获取数据，并写到Druid中。
...
在datasource中,windowPeriod设置成了P10M,timestamp不在当前时间10M内的数据都会被过滤，由于上面的数据的timestamp和执行时间相差了大概26分钟左右，所以都会被drop掉，为了达到演示效果，可以对bin/generate-example-metrics-main 的脚本进行调整。代码如下：
