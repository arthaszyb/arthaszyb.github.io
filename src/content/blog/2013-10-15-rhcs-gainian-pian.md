...
title: RHCS 概念篇
date: '2013-10-15'
description: Red Hat Cluster Suite (RHCS) 的架构、三大核心功能（高可用、负载均衡、存储集群）、主要组件（CMAN、DLM、CCS、Fence、GFS、LVS、LUCI）和运行原理。
category: web-infra
tags:
  - 集群
  - 高可用
...
RHCS 是一个集群工具的集合，主要由以下部分组成：

**核心组件：**

- **集群构架管理器** — RHCS 集群的基础套件，提供集群基本功能，使各节点组成集群工作。具体包含分布式集群管理器（CMAN）、成员关系管理、锁管理（DLM）、配置文件管理（CCS）、栅设备（FENCE）。
- **高可用服务管理器** — 提供节点服务监控和服务故障转移功能，当一个节点服务出现故障时，将服务转移到另一个健康节点。
- **集群配置管理工具** — RHCS 最新版本通过 LUCI 来配置和管理集群，LUCI 是基于 Web 的集群配置工具，可轻松搭建功能强大的集群系统。
- **Linux Virtual Server (LVS)** — 开源负载均衡软件，可根据指定的负载策略和算法合理分配客户端请求到各个服务节点，实现动态、智能的负载分担。

**补充组件：**

- **Red Hat GFS (Global File System)** — 红帽公司开发的集群文件系统，最新版本为 GFS2。允许多个服务同时读写一个磁盘分区，实现数据集中管理。GFS 需要 RHCS 底层组件支持。
- **Cluster Logical Volume Manager (CLVM)** — LVM 的扩展，允许集群中的机器使用 LVM 管理共享存储。
- **iSCSI** — 基于 Internet 协议的数据块传输标准，基于 IP Storage 理论的新型存储技术，RHCS 可通过 iSCSI 导出和分配共享存储。
- **Global Network Block Device (GNBD)** — GFS 的补充组件，用于 RHCS 分配和管理共享存储。GNBD 分为客户端和服务端，服务端允许导出多个块设备，客户端可导入这些设备。因 GNBD 已停止开发，使用越来越少。
四、 RHCS集群结构
...
高可用管理主要用来监督、启动和停止集群的应用、服务和资源。它提供了对集群服务的管理能力，当一个节点的服务失败时，高可用性集群服务管理进程可以将服务从失败节点转移到其他健康节点上，这种服务转移能力是自动、透明的。

RHCS 通过 rgmanager 来管理集群服务，rgmanager 运行在每个集群节点上，对应的进程为 clurgmgrd。

在一个 RHCS 集群中，高可用包含集群服务和集群资源两个方面。集群服务就是应用服务（如 apache、mysql 等），集群资源有很多种（如 IP 地址、运行脚本、ext3/GFS 文件系统等）。
