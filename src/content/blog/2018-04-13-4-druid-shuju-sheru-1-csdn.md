...
【1】Data Formats http://druid.io/docs/0.10.1/ingestion/data-formats.html
1. 摄入规范化数据：JSON、CSV、TSV
2. 自定义格式：Regex parser or the JavaScript parsers 来解析数据
3. 其他格式：http://druid.io/docs/0.10.1/development/extensions.html

【2】Configuration
对数据格式进行配置dataSchema中的parseSpec字段，具体见：http://druid.io/docs/0.10.1/ingestion/data-formats.html
...
| metricsSpec | JSON Object array | aggregators列表 | yes |
| granularitySpec | JSON Object | 如何建立segments，如何上卷数据 | yes |
...
| Field | Type | Description | Required |
|---|---|---|---|
| type | String | 一般为string，或在Hadoop indexing job中使用hadoopyString | no |
| parseSpec | JSON Object | 标识格式format和timestamp、dimensions | yes |

ParseSpec 两个功能:
1. String Parser用parseSpec判定将要处理rows的数据格式（JSON, CSV, TSV）
2. 所有的Parsers 用parseSpec判定将要处理rows的 timestamp 和 dimensions
...
| flattenSpec | JSON Object | 标识嵌套JSON如何打平的配置，详见Flattening JSON | no |
...
使用String Parser 加载CSV，Strings用net.sf.opencsv library解析。
...
| timestampSpec | JSON Object | timestamp的列和format | yes |
| dimensionsSpec | JSON Object | 数据的dimensions | yes |
| listDelimiter | String | 多值dimensions的分割符 | no (default == ctrl+A) |
| columns | JSON array | 数据列 | yes |
...
| Field | Type | Description | Required |
|---|---|---|---|
| dimensions | JSON array | dimension schema对象或dimension names，标识维度列，否则将timestamp列外的所有string列作为维度列 | yes |
| dimensionExclusions | JSON String array | ingestion之外的dimensions | no (default == []) |
...
granularity spec 默认是uniform，可以通过type字段配置，目前支持uniform和arbitrary types。
...
| Field | Type | Description | Required |
|---|---|---|---|
| segmentGranularity | string | 建立segments的周期 | no (default == 'DAY') |
| queryGranularity | string | 可query结果的最小granularity，数据以这个granularity在segment中聚合。例如："minute" 说明data以分钟级别的granularity聚合，也就是当(minute(timestamp), dimensions)tuple中有collisions时，将用aggregators聚合值，而不是对各个rows排序 | no (default == 'NONE') |
| rollup | boolean | rollup or not | no (default == true) |
| intervals | string | raw data摄入的intervals列表，对于real-time摄取忽略 | yes for batch, no for real-time |
...
Druid将规范化后的数据分为3类：a timestamp, a dimension, or a measure (or a metric/aggregator as they are known in Druid)。更多信息：
...
数据类型的维度(Long or Float)必须在dimensionsSpec中标识，否则默认是字符串，数值型列在group时快，但由于没有索引在过滤时慢，详见Dimension Schema。

### High cardinality dimensions (e.g. unique IDs)

实际中count-distinct不需要，对IDs列排序将杀掉roll-up，影响压缩，再aggregations带着排序的IDS，增加性能减少存储，Druid's hyperUnique aggregator基于Hyperloglog，详见here。
...
dimensions在spec缺失时，所有非timestamp的列作为string型作为维度。
...
Druid是批量构建索引的（either Hadoop-based or IndexTask-based），保证interval-by-interval间的原子性更新，例如直到2015-01-01/2015-01-02间隔内的v2 segments加载到集群中后，queries才不再使用v1 segments，此时v1从集群中卸载。

updates是跨过多个segment的，指示在每个interval内是原子性的，不是整个更新的，如下：
...
datasource的segments可以有不同的schemas，如果一个string column (dimension) 在一个segment A中存在，另一个B不存在，认为B中该维度为null。对于numeric column，Aggregations跳过这条。
