...
title: RHCS5 部署
date: '2013-10-17'
description: Web+MySQL 双机高可用集群的部署实例，涵盖环境准备、RHCS 软件安装、Luci 配置、集群创建、Failover Domain 和 Resources 配置、GFS2 文件系统设置、Quorum Disk 配置等完整流程。
category: web-infra
tags:
  - 集群
  - 高可用
...
二、 安装前准备工作
CentOS 是 RHEL 的克隆版本，且 RHCS 所有功能组件都免费提供，以下基于 CentOS。

操作系统：统一采用 CentOS 5.3 版本。为了方便安装 RHCS 套件，在安装操作系统时建议选择以下安装包：
...
点击 cluster，然后在 Cluster list 中点击”mycluster”，接着在左下端的 mycluster 栏中点击 Failover Domains --> Add a Failover Domain。图7所示界面中各参数含义：

- **Failover domain name** — 创建的失败转移域名称，起易记名字即可。
- **Prioritized** — 是否在 Failover domain 中启用域成员优先级设置，这里选择启用。
- **Restrict Failover to this domain’s member** — 是否在失败转移域成员中启用服务故障切换限制，这里选择启用。
- **Do not fail back services in this domain** — 表示在这个域中使用故障切回功能。主节点故障时，备用节点自动接管主节点的服务和资源；当主节点恢复正常时，集群的服务和资源自动从备用节点切换回主节点。
...
- `-p lock_dlm` — 定义为 DLM 锁方式。不加此参数时，两个系统同时挂载此分区会像 EXT3 一样，信息不能同步。
- `-t mycluster:my-gfs2` — 指定 DLM 锁所在的表名称，mycluster 必须与 cluster.conf 文件中 Cluster 标签的 name 值相同。
- `-j 4` — 设定 GFS2 文件系统最多支持多少个节点同时挂载，可通过 gfs2_jadd 命令在使用中动态调整。
...
- **Interval** — 间隔多长时间执行一次检查评估，单位秒。
- **Votes** — 指定 qdisk 分区投票值。
- **TKO** — 允许检查失败的次数。一个节点在 TKO*Interval 时间内连接不上 qdisk 分区，则认为此节点失败，从集群中隔离。
- **Minimum Score** — 指定最小投票值。
- **Label** — Qdisk 分区的卷标名（创建 qdisk 时指定，如”myqdisk”）。建议用卷标名，因为设备名可能在系统重启后变化，但卷标名不变。
- **Device** — 指定共享存储在节点中的设备名。
- **Path to program** — 配置第三方应用程序扩展节点状态检测精度，如 ping 命令。
