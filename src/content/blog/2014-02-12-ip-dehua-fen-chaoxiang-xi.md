...
date: '2014-02-12'
description: >-
  我们都知道，ＩＰ是由四段数字组成，在此，我们先来了解一下3类常用的ＩＰ A类IP段 0.0.0.0 到127.255.255.255 B类IP段
  128.0.0.0 到191.255.255.255 C类IP段 192.0.0.0 到223.255.255.255
...
#####################我的结论---周洋#############
若要两个IP地址在同一网段，则需要该类别级别网络标识一致才行。
A类网址需要IP地址的第一段网络标识一致即可 默认网关255.0.0.0.
B类网址要前两段网络标识相等即可 255.255.0.0
C类网址要前三段网络标识相等 255.255.255.0
网络标识二进制表示=IP地址二进制表示 AND 子网掩码二进制表示
二进制AND算法是0+0=0，1+1=1，0+1=0
```bash
ip地址 AND 255 =IP地址 （即不变）
IP地址 AND 0 =0 （即结果为0）
```
故掩码为255的段需要IP相等，而掩码为0的段ip随意即为同一网段。eg：掩码255.255.0.0的IP只要前两段一致即在同一网段。
子网掩码二进制表示中最后有n位0，则该掩码支持同网段ip数量为2的n次方个。
eg：ip为192.168.34.12，掩码为255.255.252.0，本掩码支持多少ip？与该ip同网段的ip分别是哪些？
回答：255.255.252.0的二进制为11111111.11111111.11111100.00000000
最后共有10位0，则支持2的10次方个ip，即1024个
该ip为C类地址。需要网络标识前三段一致
192.168.34.12 11000000.10101000.00100010.00001100
255.255.252.0 11111111.11111111.11111100.00000000
----------------------------------------------------------------------------------------------AND
网络标识 11000000.10101000.001000
00.00000000
红色标识的为随意数字都可（因为任何数AND 00都为0），所以如下数字符合：
11000000.10101000.00100010.任意 = 192.168.34.
11000000.10101000.00100011.任意 = 192.168.35.
11000000.10101000.00100001.任意 = 192.168.33.
11000000.10101000.00100000.任意 = 192.168.32
即32，33，34，35网段的都是同一网段。共（256-2）*4=1024个，恰好等于2的10次方。
