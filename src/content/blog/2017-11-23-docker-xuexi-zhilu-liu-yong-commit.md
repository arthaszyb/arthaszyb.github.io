...
Dockerfile更强大、灵活，推荐使用。一般来说不是真的"创建"新镜像，而是基于一个已有的基础镜像，比如Ubuntu、Fedora等，构建新的镜像而已。从零构建一个全新的镜像可参考这篇文章。

## 用commit创建镜像

### 创建Docker账号

共享和发布镜像时构建镜像中重要的环节，可以将镜像推送到Docker Hub或自己的私有Registry中。首先到 https://hub.docker.com/account/signup/ 创建账号。
...
这条命令会完成登录，并将认证信息保存起来供后面使用。个人认证信息将保存到 `$HOME/.dockercfg` 文件中。

### 用 commit 命令创建镜像

可以想象是往版本控制系统里提交变更：首先创建一个容器，并在容器里做修改，就像修改代码一样，最后再将修改提交为一个新镜像。
...
所以先用 `exit` 命令退出容器，再运行 `docker commit` 命令：

```bash
$ sudo docker commit 614122c0aabb aoct/apache2
```

命令中，指定了要提交的修改过的容器的ID、目标镜像仓库、镜像名。commit提交的其实是创建容器的镜像与容器的当前状态之间的差异部分，很轻量。
...
上面代码，用 `-m` 指定新创建的镜像的提交信息，`--author` 指定镜像作者，接着是容器ID、目标镜像仓库、镜像名。

使用 `docker inspect` 命令查看新创建的镜像的详细信息：

```bash
$ sudo docker inspect aoct/apache2
```

使用 `docker run` 命令从刚创建的新镜像运行一个容器：
