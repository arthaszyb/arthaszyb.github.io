...
1. rhel6.domain.com上sda1和sda2为/分区和swap分区，尚余一部分磁盘空间未分配，用fdisk分出sda3一个分区（10G）准备用作iscsi共享存储。
...
1. 登录管理服务器的luci界面，端口8084：
   <!-- image lost in migration: luci1.png -->
2. 输入用户名root和密码，登录
   <!-- image lost in migration: luci2.png -->
3. 转到manage cluster界面
   <!-- image lost in migration: luci3.png -->
4. 点击Create,创建集群
   <!-- image lost in migration: luci4.png -->
5. 此处密码为ricci密码
   <!-- image lost in migration: luci5.png -->
6. 集群创建成功
   <!-- image lost in migration: luci6.png -->
7. 转到Fence Devices创建fence设备，因为不是用的服务器，没有fence设备，我随意建了一个SRF，实际生产环境中，必须使用fence硬件设备。
   <!-- image lost in migration: luci7.png -->
8. 回到nodes菜单下，点击v1.domain.com,出现节点详情后点击add fence method添加一个method，
9. 点击add fence instance，添加刚才设定的SRF。
   <!-- image lost in migration: luci8.png -->
   <!-- image lost in migration: luci8-2.png -->
   <!-- image lost in migration: luci8-3.png -->
   <!-- image lost in migration: luci9.png -->
...
1. 在将lvm卷lv1格式化成GFS2文件系统
   ...
   ```
   -p指定lock参数，-t指定集群名：文件系统名，-j指定可以连接的节点数
2. 节点上挂载GFS2文件系统
   ```bash
...
1. 修改两个节点上的/etc/httpd/conf/httpd.conf
   ...
   ```
   DocumentRoot "/var/www/html"改为DocumentRoot "/mnt/data1/www1"
   ```bash
   [root@v2 ~]# vim /etc/httpd/conf/httpd.conf
   ```
   DocumentRoot "/var/www/html"改为DocumentRoot "/mnt/data1/www2"
2. 新建目录及网页文件index.html
   ```bash
   [root@v2 ~]# mkdir /mnt/data1/www1 /mnt/data1/www2
...
4. 访问192.168.137.11和192.168.137.12得到：
   <!-- image lost in migration: 11.png -->
   <!-- image lost in migration: 12.png -->
...
1. 创建Web高可用需要的两个资源：
   ...
   <!-- image lost in migration: 521.png -->
   <!-- image lost in migration: 522.png -->
2. 新建故障倒换域
   <!-- image lost in migration: 523.png -->
3. 新建服务组，添加刚才新建的两个资源进服务组
   <!-- image lost in migration: 524.png -->
   <!-- image lost in migration: 525.png -->
4. 启动服务组
   <!-- image lost in migration: 526.png -->

   显示运行在v2上，访问192.168.137.10有：
   <!-- image lost in migration: 527.png -->

### 5.3 高可用性验证

1. 手动切换
   <!-- image lost in migration: 531.png -->
   <!-- image lost in migration: 532.png -->

   刷新192.168.137.10页面
   <!-- image lost in migration: 533.png -->
2. 模拟宕机，关闭V1（现实生产应用中必须使用Fence设备！！！）
   <!-- image lost in migration: 534.png -->
